<script>
	...
	$: buttons = isDesktop ? slice(pageCount, $pageNumber) : [0, 1, 2]
...
{:else}
	...
	>
		...
		{#if pageCount.length > 4 && $pageNumber > 3}
			<button class="ellipse">...</button>
		...
		{#if pageCount.length > 4 && $pageNumber <= pageCount.length - 1}
			<button class="ellipse">...</button>
		{/if}
